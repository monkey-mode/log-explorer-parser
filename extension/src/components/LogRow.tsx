...
const SEV_STYLE: Record<string, {
  border: string; dot: string; row: string; msg: string;
}> = {
  ERROR: { border: 'border-l-red-500',    dot: 'bg-red-500',    row: 'hover:bg-red-950/30',    msg: 'text-red-200'    },
  WARN:  { border: 'border-l-yellow-400', dot: 'bg-yellow-400', row: 'hover:bg-yellow-950/20', msg: 'text-yellow-100' },
  INFO:  { border: 'border-l-slate-700',  dot: 'bg-blue-400',   row: 'hover:bg-slate-800/50',  msg: 'text-slate-200'  },
  DEBUG: { border: 'border-l-slate-800',  dot: 'bg-slate-600',  row: 'hover:bg-slate-800/40',  msg: 'text-slate-400'  },
};
      ...
      {expanded && (
        <div className="bg-slate-900/70 border-t border-slate-800 px-4 pt-3 pb-4">
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 mb-3 text-[11px]">
            <span><span className="text-slate-600">time  </span><span className="text-slate-300">{(log.payload.timestamp as string) ?? log.ts}</span></span>
            <span><span className="text-slate-600">svc   </span><span className="text-cyan-400">{log.container}</span></span>
            {log.corrId    && <span><span className="text-slate-600">corr  </span><span className="text-blue-300 break-all">{log.corrId}</span></span>}
            {log.requestId && <span><span className="text-slate-600">req   </span><span className="text-slate-300 break-all">{log.requestId}</span></span>}
            ...
            {p.error && <span><span className="text-slate-600">error </span><span className="text-red-300 break-all">{p.error as string}</span></span>}
          </div>
          ...
          {p.stacktrace && (
            <div className="mt-2 rounded border border-red-900/40 overflow-hidden">
              <div className="px-3 py-1 bg-red-950/30 border-b border-red-900/40 text-[10px] text-red-400">stacktrace</div>
